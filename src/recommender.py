...
from .db_functions import (
    get_engine,
    search as postgresql_vec_search,
    get_embedding_count,
    table_exists,
    get_movie_count,
...
class MovieRecommender:
    ...
    def _load_metadata(self):
        ...
        if not self.genre_classes:
            raise Exception(
                "Genre classes metadata not found! Please run 'python load_embeddings.py' first."
            )
        ...
        if not year_stats:
            raise Exception(
                "Year statistics metadata not found! Please run 'python load_embeddings.py' first."
            )
        ...
        if not self.text_metadata:
            raise Exception(
                "Text embedding metadata not found! Please run 'python load_embeddings.py' first."
            )
        ...
        if not self.person_metadata:
            raise Exception(
                "Person embedding metadata not found! Please run 'python load_embeddings.py' first."
            )
        ...
        print(
            f"✓ Loaded sentence transformer models: {self.text_metadata['model_name']}"
        )
    ...
    def recommend(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        # Build text input from query
        text_input = query

        # Get text embedding
        text_embedding = self._build_text_vector(text_input)

        # Search for similar movies
        similar_movies = postgresql_vec_search(text_embedding, top_n=limit)
        ...
        movie_details = get_movie_details_by_ids(movie_ids)
            ...
            recommendations.append(
                {
                    ...
                    "year": str(movie["start_year"]) if movie["start_year"] else None,
                    "genres": movie["genres"],
